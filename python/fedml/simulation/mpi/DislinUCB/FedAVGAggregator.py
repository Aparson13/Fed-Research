...
########################################## End
    ...
    def aggregate(self, pool_articles, clientID):
        start_time = time.time()
        model_list = []

        for idx in range(self.worker_num):
            model_list.append((self.model_dict[idx])) # self.sample_num_dict[idx],
            # training_num += self.sample_num_dict[idx]
        logging.info("len of self.model_dict[idx] = " + str(len(self.model_dict)))

        # def decide(self, pool_articles, clientID):
        # if clientID not in self.clients:
        #     self.clients[clientID] = DislinUCBTrainer(self.dimension, self.lambda_, self.delta_, self.NoiseScale)
        ...
        self.set_global_model_params(articlePicked)
