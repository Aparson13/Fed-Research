...
class AsyncDisLinUCBClientManager(FedMLCommManager):
    def __init__(
        self,
        args,
        trainer,
    ...
    def __train(self, global_model_params, client_index):
        ...
        A_upload_buffer, b_upload_buffer, local_sample_num = self.trainer.train(self.round_idx)
        ...
        if self.trainer.uploadTrigger():
            # diff_weights = get_name_params_difference(global_model_params, weights) # weights - global_model_params
            self.send_result_to_server(0, A_upload_buffer, b_upload_buffer, local_sample_num, client_runtime_info)
